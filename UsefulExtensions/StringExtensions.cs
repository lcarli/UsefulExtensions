...
namespace UsefulExtensions
{
    public static class StringExtensions
    {
        ...
        public static TTarget? To<TTarget>(this string instance) where TTarget : struct
        {
            instance = (instance ?? string.Empty).ToLower();
            var targeType = typeof(TTarget);
            MethodInfo tryParseMethodInfo = GetTryParseMethod<TTarget>(targeType);
            ...
            var success = (bool)tryParseMethodInfo.Invoke(null, args);
        ...
        public static TTarget To<TTarget>(this string instance, TTarget defaultValue) where TTarget : struct
        {
            return instance.To<TTarget>() ?? defaultValue;
        }

        private static MethodInfo GetTryParseMethod<TTarget>(Type targeType) where TTarget : struct
        {
            return targeType.GetMethod("TryParse",
        ...
        private static string ParseBoolean(string instance, Type type)
        {
            if (type != typeof(bool)) return instance;

            string[] trueWords = { "1", "on", "yes", "success", "sim", "sucesso", "ligado" };

            return trueWords.Contains(instance) ? "true" : "false";
        }
        ...
        public static string GetMimeTypeFromExtension(this string fileExtension)
        {
            ...
            return Mappings.TryGetValue(fileExtension, out var mime) ? mime : "application/octet-stream";
        }
